...
# Full Release Pipeline for distributed
...
FORMULA_PATH="$HOMEBREW_TAP_PATH/Formula/distributed.rb"
...
main() {
    echo ""
    echo "========================================="
    echo "   distributed Release Pipeline"
    echo "========================================="
    echo ""
    ...
    # Step 6: Update formula with new URL and SHA256 (if formula exists)
    if [[ -f $FORMULA_PATH ]]; then
        ...
        git add Formula/distributed.rb
        git commit -m "Release distributed ${NEW_VERSION}"
        git push
        cd - > /dev/null

        print_success "Homebrew formula updated in tap"
    else
        print_warning "No Homebrew formula found - skipping tap update"
    fi
